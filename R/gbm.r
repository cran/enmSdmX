# NB These functions have been borrowed from dismo so that its dependency on outdated PROJ can be obviated. I'd love to call them from another source if/when possible. - Adam
#
...
# and then do the same for absences
			...
			selector <- selector[order(stats::runif(n.cases, 1, 100))]
		}
    } else {
...
# now begin process of calculating optimal number of trees
	...
	cv.loss.ses <- sqrt(apply(cv.loss.matrix,2,stats::var)) / sqrt(n.folds)
...
# plot out the resulting curve of holdout deviance 
	if (plot.main) {
		...
		graphics::abline(h = y.bar, col = 2)

		graphics::lines(trees.fitted, cv.loss.values + cv.loss.ses, lty=2)  
		graphics::lines(trees.fitted, cv.loss.values - cv.loss.ses, lty=2)  

		if (plot.folds) {
			for (i in 1:n.folds) {
				graphics::lines(trees.fitted, cv.loss.matrix[i,],lty = 3)
			}
		}
		graphics::abline(v = target.trees, col=3)
		graphics::title(paste(sp.name,", d - ",tree.complexity,", lr - ",learning.rate, sep=""))
	}
...
# includes estimates of the standard error of the fitted values added 2nd may 2005
	...
	for (i in 1:n.folds) {
		...
		cv.cor.stats[i] <- stats::cor(y_i,u_i)

		if (family == "bernoulli") {
			cv.roc.stats[i] <- .roc(y_i,u_i)
			cv.calibration.stats[i,] <- .calibration(y_i,u_i,"binomial")
			threshold.stats[i] <- stats::approx(stats::ppoints(u_i), sort(u_i,decreasing = T), prevalence)$y
		}
	...
	fitted.vars <- apply(fitted.matrix,1, stats::var, na.rm = TRUE)

# now calculate the mean and se's for the folds

	cv.dev <- mean(cv.deviance.stats, na.rm = TRUE)
	cv.dev.se <- sqrt(stats::var(cv.deviance.stats)) / sqrt(n.folds)

	cv.cor <- mean(cv.cor.stats, na.rm = TRUE)
	cv.cor.se <- sqrt(stats::var(cv.cor.stats, use = "complete.obs")) / sqrt(n.folds)
	...
	if (family == "bernoulli") {
		...
		cv.roc.se <- sqrt(stats::var(cv.roc.stats, use = "complete.obs")) / sqrt(n.folds)  
		cv.threshold <- mean(threshold.stats, na.rm = T)
		cv.threshold.se <- sqrt(stats::var(threshold.stats, use = "complete.obs")) / sqrt(n.folds)  
	}
	...
	if (family == "poisson" | family == "bernoulli") {
		...
		cv.calibration.se <- apply(cv.calibration.stats,2,stats::var)
...
#
	...
	if(family == "binomial") {
		pred <- preds + 1e-005
		pred[pred >= 1] <- 0.99999
		mod <- stats::glm(obs ~ log((pred)/(1 - (pred))), family = stats::binomial)
		lp <- log((pred)/(1 - (pred)))
		a0b1 <- stats::glm(obs ~ offset(lp) - 1, family = stats::binomial)
		miller1 <- 1 - stats::pchisq(a0b1$deviance - mod$deviance, 2)
		ab1 <- stats::glm(obs ~ offset(lp), family = stats::binomial)
		miller2 <- 1 - stats::pchisq(a0b1$deviance - ab1$deviance, 1)
		miller3 <- 1 - stats::pchisq(ab1$deviance - mod$deviance, 1)
	} else if(family == "poisson") {
		mod <- stats::glm(obs ~ log(preds), family = stats::poisson)
		lp <- log(preds)
		a0b1 <- stats::glm(obs ~ offset(lp) - 1, family = stats::poisson)
		miller1 <- 1 - stats::pchisq(a0b1$deviance - mod$deviance, 2)
		ab1 <- stats::glm(obs ~ offset(lp), family = stats::poisson)
		miller2 <- 1 - stats::pchisq(a0b1$deviance - ab1$deviance, 1)
		miller3 <- 1 - stats::pchisq(ab1$deviance - mod$deviance, 1)
	}
