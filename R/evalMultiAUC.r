...
evalMultiAUC <- function(
	...
	# multivariate AUC
	evalMultiAUC <- if (weightBySize) {
		sum(aucs * numCases) / sum(numCases)
	} else {
		mean(aucs)
	}
		
	aucs <- c(aucs, evalMultiAUC)
