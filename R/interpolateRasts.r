...
#'
#' @param rasts A "stack" of \code{SpatRasters}s.
...
interpolateRasts <- function(
	...
	####################
		...
		if (terra::nlyr(rasts) < 2) stop('Argument "rasts" must have >1 raster layer.')
		if (length(interpFrom) != terra::nlyr(rasts)) stop('Argument "interpFrom" must have same length as number of rasters in argument "rasts".')
	...
	##################################
		...
		if (useRasts) {
			out <- terra::rast(thisOut, extent=extent, crs=crs)
			rm(thisOut)
		}
	...
	#########################
		...
		if (verbose) progress <- utils::txtProgressBar(min=1, max=terra::ncell(rasts), style=3, width=min(64, getOption('width')))
			...
			for (countCol in 1:ncol(rasts)) {
				...
				if (sum(!is.na(y)) < 2) {
					...
					if (type == 'gam') {
						...
						interpModel <- tryCatch(
							stats::smooth.spline(
								x = dataFrom$x,
								y = dataFrom$y,
								keep.data = FALSE,
								control.spar = list(trace=FALSE)
					...
					if (!is.logical(interpModel)) {
						...
						cellInterpol <- do.call('predict', args=list(object=interpModel, newdata=dataTo, x = dataTo, type='response'))
